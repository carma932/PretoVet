    ...
    <div class="py-10">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-300/50 shadow-xl rounded-2xl p-8">
                <form method="POST" action="{{ route('productos.update', $producto->codigo) }}" class="space-y-3">
                    @csrf
                    @method('PUT')
                    ...
                    <select name="tipo"
                        ...
                        <option value="Objetos" {{ $producto->tipo == 'Objetos' ? 'selected' : '' }}>Objetos</option>
                        <option value="Comida" {{ $producto->tipo == 'Comida' ? 'selected' : '' }}>Comida</option>
                    </select>
